...
app.post('/dining', async (req: Request, res: Response) => {
    ...
    try {
        const response = await fetch('https://api.elevate-dxp.com/api/mesh/c087f756-cc72-4649-a36f-3a41b700c519/graphql?' + queryStr, {
            headers
        });
...
app.listen(PORT, () => {
    ...
    console.log(`Go to https://studio.apollographql.com/sandbox/explorer and type "http:/localhost:4000/dining" as the endpoint.`);
});
